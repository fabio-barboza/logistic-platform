...
Regra estruturante: **a LLM nunca toca o banco.** Ela chama tools MCP expostas pela `logistic-api`; só a API executa SQL sobre o domínio (motorista, veículo, pedido, rota). O `logistic-agent` tem datasource, mas apontando para um **banco exclusivo dele** (`agentdb`, container próprio em 5433) e guardando só o estado da conversa — histórico de chat, pendência de confirmação, oferta/intenção pendentes. Ele não tem credencial no `logisticdb`: dado de domínio continua sendo tool MCP nova na API, nunca repositório no agent, e agora isso é imposto pelo banco e não pela convenção.
