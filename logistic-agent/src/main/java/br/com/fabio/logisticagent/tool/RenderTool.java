...
public class RenderTool {
    ...
    public String renderChart(
        ...
        if (labels == null || labels.isEmpty()) {
            return "labels está vazio. Envie um rótulo por categoria, ex: [\"SP\",\"RJ\",\"MG\"].";
        }
        if (datasets == null || datasets.isEmpty()) {
            return "datasets está vazio. Envie ao menos uma série, ex: [{\"label\":\"Pedidos\",\"data\":[42,30,25]}].";
        }
        for (Dataset dataset : datasets) {
            if (dataset.data() == null || dataset.data().isEmpty()) {
                return "O dataset '" + dataset.label() + "' está sem data. Envie um número por rótulo de labels.";
            }
            if (dataset.data().size() != labels.size()) {
                return "O dataset '" + dataset.label() + "' tem " + dataset.data().size()
    ...
    public String renderTable(
        ...
        if (columns == null || columns.isEmpty()) {
            return "columns está vazio. Envie os nomes das colunas, ex: [\"Estado\",\"Entregas\"].";
        }
        if (rows == null || rows.isEmpty()) {
            return "rows está vazio. Envie uma linha por registro, com um valor por coluna.";
        }
        for (int i = 0; i < rows.size(); i++) {
            List<String> row = rows.get(i);
            if (row == null || row.size() != columns.size()) {
                return "A linha " + (i + 1) + " tem " + (row == null ? 0 : row.size())
